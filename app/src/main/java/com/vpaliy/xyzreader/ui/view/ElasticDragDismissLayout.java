...
public class ElasticDragDismissLayout extends FrameLayout {
    ...
    public ElasticDragDismissLayout(Context context, AttributeSet attrs) {
    ...
    public void onStopNestedScroll(View child) {
        if (Math.abs(totalDrag) >= dragDismissDistance) {
            ...
            animate()
                    ...
                    .scaleY(1f)
                    .setDuration(200L)
                    .setInterpolator(TransitionUtils.getFastOutSlowInInterpolator(getContext()))
                    .setListener(null)
                    .start();
            totalDrag = 0;
    ...
    public static class SystemChromeFader extends ElasticDragDismissCallback {
        ...
        public SystemChromeFader(Activity activity) {
            ...
            fadeNavBar = TransitionUtils.isNavBarOnBottom(activity);
        }
        ...
        public void onDrag(float elasticOffset, float elasticOffsetPixels,
            ...
            if (elasticOffsetPixels > 0) {
                ...
                activity.getWindow().setStatusBarColor(TransitionUtils.modifyAlpha(activity.getWindow()
                        .getStatusBarColor(), (int) ((1f - rawOffset) * statusBarAlpha)));
            } else if (elasticOffsetPixels == 0) {
                // reset
                activity.getWindow().setStatusBarColor(TransitionUtils.modifyAlpha(
                        activity.getWindow().getStatusBarColor(), statusBarAlpha));
                activity.getWindow().setNavigationBarColor(TransitionUtils.modifyAlpha(
                        activity.getWindow().getNavigationBarColor(), navBarAlpha));
            } else if (fadeNavBar) {
                // dragging upward, fade the navigation bar in proportion
                activity.getWindow().setNavigationBarColor(
                        TransitionUtils.modifyAlpha(activity.getWindow().getNavigationBarColor(),
